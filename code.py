    ...
    def get_next_token(self):
        ...
        while self.current_token is not None:
            ...
            if self.current_token == 'while':
                self.next()
                return Tokenizer(WHILE, 'while')
            if self.current_token == 'do':
                self.next()
                return Tokenizer(DO, 'do')
            ...
            if self.current_token == 'then':
                self.next()
                return Tokenizer(THEN, 'then')
            if self.current_token == 'else':
                self.next()
                return Tokenizer(ELSE, 'else')

            if self.current_token == 'skip':
                self.next()
                return Tokenizer(SKIP, 'skip')

            if self.is_integer(self.current_token):
                the_token = Tokenizer(INTEGER, int(self.current_token))
                self.next()
                return the_token
            ...
            if self.current_token == '(':
                self.next()
                return Tokenizer(LPAREN, '(')

            if self.current_token == ')':
                self.next()
                return Tokenizer(RPAREN, ')')
            ...
            if self.current_token == 'true':
                self.next()
                return Tokenizer(TRUE, 'true')

            if self.current_token == 'false':
                self.next()
                return Tokenizer(FALSE, 'false')
            if self.current_token == '¬':
                self.next()
                return Tokenizer(NOT, '¬')

            if self.current_token.isidentifier():                
                tk = Tokenizer(ID, self.current_token)
                self.next()
                return tk
...
class Parser(object):
    ...
    def bool_factor(self):
        ...
        if self.current_token.type == TRUE:
            #print ('Boolean','true')
            node = Boolean(Number(Tokenizer(INTEGER,1)), Tokenizer(EQUAL,'='), Number(Tokenizer(INTEGER,1)))
            return node
        elif self.current_token.type == FALSE:
            node = Boolean(Number(Tokenizer(INTEGER,1)), Tokenizer(EQUAL,'='), Number(Tokenizer(INTEGER,2)))
            return node
        elif token.type == NOT:
            self.eat(NOT)
